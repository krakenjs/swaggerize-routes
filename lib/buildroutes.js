...
function buildSecurity(basedir, securityDefinitions, routeSecurity) {
    if (!securityDefinitions || !routeSecurity) {
        return undefined;
    }

    Object.keys(routeSecurity).forEach(function (name) {
        var scopes;

        scopes = routeSecurity[name].scopes;

        assert.ok(securityDefinitions[name], 'Unrecognized security definition (' + name + ')');

        scopes.forEach(function (scope) {
            assert.ok(thing.isString(scope) && securityDefinitions[name].scopes.indexOf(scope) > -1, 'Unrecognized scope (' + scope + ').');
        });

        if (securityDefinitions[name]['x-authorize']) {
            routeSecurity[name].authorize = resolve(basedir, securityDefinitions[name]['x-authorize']);
        }
    });
