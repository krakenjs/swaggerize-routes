...
const Swaggerize = ( options = {}, callback) => {
    ...
    //api should be a valid object
    Assert.ok(isObject(api), 'Expected an api definition.');
    ...
    routes = Buildroutes((!validated) ? Parser.validate(api) : Promise.resolve(api), options);

    return Maybe(callback, routes);
}
